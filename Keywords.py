...
class Keywords:
    ...
    def __get_keywords_short(self, tokens, top_n=20):
        ...
        for token in token_frequency:
            ...
            current_corpus_frequency = 0.0001
        ...
        top_tokens_without_compound_parts = (
            self.__remove_versions_of_compound_words_from_list(
                top_tokens_with_compound_parts
            )
        )

        results = KeywordResults(
            top_tokens_without_compound_parts, str(token), tokens.get_top_grams_list()
        )
        return results
    ...
    def __get_keywords_frequent_tokens(self, tokens, top_n=20):
        ...
        top_keywords = self.__remove_versions_of_compound_words_from_list(
            top_score_keywords
        )
        no_empty_strings = [w for w in top_keywords if self.__token_has_text(w)]

        results = KeywordResults(
            no_empty_strings[:max_keywords], str(token), tokens.get_top_grams_list()
        )
        return results
